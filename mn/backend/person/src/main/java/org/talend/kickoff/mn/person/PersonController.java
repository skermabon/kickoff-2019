...
@Controller("/person/v1/persons") public class PersonController implements PersonOperations {
    ...
    @Override public HttpResponse<Person> get(@PathVariable String id) {
    ...
    @Override public HttpResponse<Person> post(@Body Person person) {
        return HttpResponse.created(personRepository.create(person).blockingGet());
    }

    @Override public HttpResponse<Person> put(@PathVariable String id, @Body Person person) {
